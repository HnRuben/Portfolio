...
// ── JAVA SPRING ──────────────────────────────────────────────────
    ...
    // ── CRÉATIVITÉ ────────────────────────────────────────────────────
    {
      ...
      definition: `Dans le monde du développement, la créativité serait pour moi la capacité à imaginer les solutions les plus adaptées à un problème quand la voie évidente ne marche pas. Sans forcément innover à chaque fois ou inventer quelque chose de zéro, mais plutôt regarder les contraintes différemment et en faire quelque chose d'utile, que ce soit dans le choix d'une architecture, la conception d'une nouvelle fonctionnalité Elle se manifeste aussi en dehors du code, comme dans la communication visuelle d'un projet, la conception d'une expérience utilisateur originale, ou l'initiative de créer quelque chose qui n'existait pas. Cette compétence selon moi peut être ce qui différencie un développeur seulement technique d'un développeur qui créé de la valeur.`,
      context: `La créativité c'est quelque chose qui revient dans beaucoup de mes projets, pas toujours de la même façon. Sur mon projet "DarwinSimulator" c'est parti d'une envie de modéliser quelque chose de concret, l'évolution darwinienne, et de le voir fonctionner. Sur mon projet "D3", l'idée de départ c'était de simuler un OS entier pour gérer un serveur Discord, ce qui a donné son identité au projet dès le début. Et en dehors du code, j'ai aussi une sensibilité au design graphique que j'ai mise en pratique sur Canva, notamment pour l'événement "Dessin Vivant".`,
      anecdotes: [
        {
  ...
  readonly realisations: Realisation[] = [
    ...
    },
    {
      ...
      results: `La simulation produite offre une expérience immersive où les comportements des NPCs évoluent de manière entièrement autonome et non scriptée. On observe l'émergence progressive de stratégies de survie optimisées sur plusieurs générations, validant le bon fonctionnement du système NEAT implémenté. Le projet a été présenté avec succès dans le cadre de notre formation. Pour moi personnellement, DarwinSimulator a été le projet le plus techniquement ambitieux de mon cursus scolaire, celui qui m'a le plus poussé à réfléchir en dehors de ma zone de confort algorithmique.`,
    ...
    },
    {
      ...
      results: `C2 est un jeu runner 2D complet et jouable, avec un gameplay fluide, des effets sonores personnalisés, des animations et un éditeur de niveaux fonctionnel permettant l'export en JSON. Le projet a été présenté avec succès aux TechDays de l'ESIEA. Pour moi, ce projet a été le premier où j'ai vraiment compris ce que ça voulait dire de concevoir une architecture logicielle propre : l'EventBus, le MVC, les responsabilités bien séparées, c'est là que ces concepts sont passés de théoriques à concrets.`,
